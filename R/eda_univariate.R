eda_univariate <- function(data = NULL,file_info = NULL,k = 3){
  ...
  mydataframe <- c("Column_Names","IQR_upper_threshold","IQR_lower_threshold","`IQR_upper_outier%`","`IQR_lower_outier%`",paste("`","m+",k,"sd_threshold","`",sep = ""),paste("`","m-",k,"sd_threshold","`",sep = ""),paste("`","m+",k,"sd_outliers%","`",sep = ""),paste("`","m-",k,"sd_outliers%","`",sep = ""),"concentration","priority","performance","notes")
    for(i in 1:length(uni$columns)){
      ...
      uni$columns[[i]] <- rbind(uni$columns[[i]],data.frame("a1" = a1,"a2"=a2,"a3"=a3,"a4"=a4,"a5"=a5,"a6"=a6,"a7"=a7,"a8"=a8,"a9"=a9,"a10" = a13,"a11"= a10,"a12" = a11,"a13" = a12))
