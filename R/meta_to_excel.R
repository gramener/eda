meta_to_excel <- function(meta_data,wb ,sheet= "Metadata"){
  start <- Sys.time()
  metadata1 <- NULL
  ...
  if(!file.exists(wb)){
    ...
    addDataFrame(data.frame(names(meta_data)[1:(length(meta_data)-1)],as.character(meta_data[1:(length(meta_data)-1)])), sheet=sheet,startRow=2,startColumn=1, row.names=FALSE,col.names = FALSE)
    saveWorkbook(wb1, wb)
  }
  else{
    wb1<-loadWorkbook(wb)
